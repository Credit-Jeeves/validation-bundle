<?php
namespace CreditJeeves\ExperianBundle\JMS;
...
class XmlSerializationVisitor extends AbstractVisitor
{
    ...
    public function visitNull($data, array $type, Context $context)
    {
        ...
        $this->currentNode->appendChild($current);
    ...
    public function visitArray($data, array $type, Context $context)
    {
        ...
        $entryName = (null !== $this->currentMetadata && null !== $this->currentMetadata->xmlEntryName) ?
            $this->currentMetadata->xmlEntryName :
            'entry';
        $keyAttributeName = (null !== $this->currentMetadata && null !== $this->currentMetadata->xmlKeyAttribute) ?
            $this->currentMetadata->xmlKeyAttribute :
            null;

        foreach ($data as $k => $v) {
            $tagName = (null !== $this->currentMetadata &&
                    $this->currentMetadata->xmlKeyValuePairs &&
                    $this->isElementNameValid($k)
                ) ? $k : $entryName;
    ...
    public function visitProperty(PropertyMetadata $metadata, $object, Context $context)
    {
        ...
        if ($metadata->xmlAttribute) {
            ...
            if (!$node instanceof \DOMCharacterData) {
                throw new RuntimeException(
                    sprintf(
                        'Unsupported value for XML attribute. Expected character data, but got %s.',
                        json_encode($v)
                    )
                );
            }
        ...
        if ($metadata->xmlValue) {
            ...
            if (!$node instanceof \DOMCharacterData) {
                throw new RuntimeException(
                    sprintf(
                        'Unsupported value for property %s::$%s. Expected character data, but got %s.',
        ...
        if ($metadata->xmlAttributeMap) {
            if (!is_array($v)) {
                throw new RuntimeException(
                    sprintf('Unsupported value type for XML attribute map. Expected array but got %s.', gettype($v))
                );
            }
            ...
            foreach ($v as $key => $value) {
                ...
                if (!$node instanceof \DOMCharacterData) {
                    throw new RuntimeException(
                        sprintf(
                            'Unsupported value for a XML attribute map value. Expected character data, but got %s.',
                            json_encode($v)
                        )
                    );
                }
        ...
        if ($addEnclosingElement) {
            ...
            if ($element->hasChildNodes() || $element->hasAttributes() ||
                (isset($metadata->type['name']) &&
                    $metadata->type['name'] === 'array' &&
                    isset($metadata->type['params'][1])
                )
            ) {
