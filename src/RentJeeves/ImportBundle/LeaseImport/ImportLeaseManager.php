...
namespace RentJeeves\ImportBundle\LeaseImport;
...
use RentJeeves\ImportBundle\LeaseImport\Extractor\ExtractorBuilder;
...
/**
 * Service`s name "import.lease.manager"
 */
class ImportLeaseManager
{
    /**
    ...
    /**
     * @param Import $import
     * @param string $additionalParameter pathToFile or extPropertyId
     *
     * @throws ImportLogicException when U use this service for Import with type != lease
     */
    public function import(Import $import, $additionalParameter)
    {
        $group = $import->getGroup();
        if ($import->getImportType() !== ImportModelType::LEASE) {
            $this->logger->warning(
                $message = sprintf(
        ...
        try {
            ...
            $this->logger->info(
                sprintf(
                    'ImportLeaseManager for Import#%d will use "%s" for extract data.',
                    $import->getId(),
                    get_class($extractor)
                ),
                ['group' => $group, 'additional_parameter' => $additionalParameter]
            ...
            if (false === empty($extractedData)) {
                ...
                $this->logger->info(
                    sprintf(
                        'ImportLeaseManager for Import#%d will use "%s" for transform data.',
                        $import->getId(),
                ...
                $this->logger->info(
                    sprintf(
                        'ImportLeaseManager for Import#%d will use "%s" for load data.',
                        $import->getId(),
                        get_class($loader)
                    ),
                    ['group' => $group, 'additional_parameter' => $additionalParameter]
