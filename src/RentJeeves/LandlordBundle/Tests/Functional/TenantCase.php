...
class TenantCase extends BaseTestCase
{
    ...
    public function approve()
    {
        ...
        $amount->setValue('200');
        $this->assertNotNull($contractEditStart = $this->page->find('css', '#contractApproveStart'));
        $contractEditStart->setValue('08/01/2013');
        $this->assertNotNull($contractEditFinish = $this->page->find('css', '#contractApproveFinish'));
        $contractEditFinish->setValue('08/25/2013');
        $this->page->pressButton('approve.tenant');
        $this->session->wait($this->timeout, "$('#processLoading').is(':visible')");
        $this->session->wait($this->timeout, "!$('#processLoading').is(':visible')");
