...
class StorageResmanCase extends BaseTestCase
{
    ...
    public function shouldNotGetMappingFromDB()
    {
        ...
        $storageResmanMock = $this->getMock(
            'RentJeeves\LandlordBundle\Accounting\Import\Storage\StorageResman',
            [],
            [],
            '',
            false
        ...
        $storageResmanMock->expects($this->any())
            ...
            ->will($this->returnValue(ResManClientCase::EXTERNAL_PROPERTY_ID));
    ...
    public function shouldGetMappingFromDB()
    {
        ...
        $importApiMapping = new ImportApiMapping();
        $importApiMapping->setMappingData([1, 2]);
        $importApiMapping->setExternalPropertyId(ResManClientCase::EXTERNAL_PROPERTY_ID);
        $importApiMapping->setHolding($landlord->getHolding());
        $this->getEntityManager()->persist($importApiMapping);
        $this->getEntityManager()->flush();

        $storageResmanMock = $this->getMockBuilder('RentJeeves\LandlordBundle\Accounting\Import\Storage\StorageResman')
