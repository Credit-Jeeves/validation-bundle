...
class StorageYardiCase extends BaseTestCase
{
    ...
    public function shouldNotGetMappingFromDB()
    {
        ...
        $storageResmanMock = $this->getMock(
            'RentJeeves\LandlordBundle\Accounting\Import\Storage\StorageYardi',
            [],
            [],
            '',
            false
        ...
        $storageResmanMock->expects($this->any())
            ...
            ->will($this->returnValue(PaymentClientCase::PROPERTY_ID));
    ...
    public function shouldGetMappingFromDB()
    {
        ...
        $propertyMapping->setExternalPropertyId(PaymentClientCase::PROPERTY_ID);
        $importApiMapping = new ImportApiMapping();
        $importApiMapping->setMappingData([1, 2]);
        $importApiMapping->setExternalPropertyId(PaymentClientCase::PROPERTY_ID);
        $importApiMapping->setHolding($landlord->getHolding());
        $this->getEntityManager()->persist($importApiMapping);
        $this->getEntityManager()->flush();

        $storageResmanMock = $this->getMockBuilder('RentJeeves\LandlordBundle\Accounting\Import\Storage\StorageYardi')
